...
{
class LogPolar: public nodelet::Nodelet
{
  // variables
public:
protected:
...
protected:
  ...
  void onInit() {
    ...
    if (use_snapshot_) {
      publish_once_ = false;
      srv_ = pnh.advertiseService("snapshot", &LogPolar::snapshot_srv_cb, this);
      sub_ = pnh.subscribe("snapshot", 1, &LogPolar::snapshot_msg_cb, this);
    }

    cp_ = it_->advertiseCamera("output/image", max_queue_size_);

    cs_ = it_->subscribeCamera("input/image", max_queue_size_,
                               &LogPolar::callback, this);
...
PLUGINLIB_DECLARE_CLASS (resized_image_transport, LogPolar, LogPolar, nodelet::Nodelet);
