...
class MaskRCNNInstanceSegmentation(ConnectionBasedTransport):
    ...
    def __init__(self):
        ...
        # if priority_id exists, hide other mask
        self.priority_id = rospy.get_param('~priority_id', -1)
        ...
        self.model = chainer_mask_rcnn.models.MaskRCNNResNet(
            n_layers=rospy.get_param('~n_layers', 50),
            n_fg_class=n_fg_class,
            pretrained_model=pretrained_model,
            anchor_scales=rospy.get_param('~anchor_scales', [4, 8, 16, 32]),
            rpn_hidden=rospy.get_param('~rpn_hidden', 1024),
            # pooling_func=chainer_mask_rcnn.functions.roi_align_2d,
    ...
    def callback(self, imgmsg):
        ...
        for ins_id, (mask, label) in enumerate(zip(masks, labels)):
            ...
            if (self.priority_id != -1) and (self.priority_id != label) :
                mask = mask * (lbl_cls != self.priority_id)
            lbl_cls[mask] = label
